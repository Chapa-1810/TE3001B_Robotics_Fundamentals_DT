robotNode = ros2node('/robot',77);
commandSub = ros2subscriber(robotNode,'/robot_command','std_msgs/String');
pause on
%%
global hand
global home
...
global old_time
old_time = 0;
disp("Ready to receive messages")
% message is like this: "movementType;direction"
while true
    [message, status, statustext] = receive(commandSub,5);
    if status
        fprintf('Received message: %s\n',message.data);
        % parse the message
        messageData = split(message.data,';');
        movementType = messageData{1};
        direction = messageData{2};
        disp("-----------------");
        fprintf('Movement type: %s\n',movementType);
        fprintf('Direction: %s\n',direction);
        disp("-----------------");
        moveRobot(movementType, direction);
        disp("Finished Movement")
...
function moveRobotSuccess = moveRobot (movementType, direction)
    global hand
    global home
    global s
    global jpos
